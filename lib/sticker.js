...
async function canvas(code, type = 'png', quality = 0.92) {
  let res = await fetch('https://iniapisiapa.herokuapp.com/api/canvas?' + queryURL({
    type,
    quality
  }), {
    method: 'POST',
...
async function addExif(webpSticker, packname, author, categories = [''], extra = {}) {
  ...
  const json = { 'android-app-store-link': 'https://github.com/caliph91/bot-whatsapp', 'sticker-pack-id': stickerPackId, 'sticker-pack-name': packname, 'sticker-pack-publisher': author, 'emojis': categories, ...extra };
